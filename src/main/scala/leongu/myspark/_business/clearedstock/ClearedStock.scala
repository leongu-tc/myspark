...
object ClearedStock extends Logging with Cons {
  ...
  val sync_list = List(
    // for 55
    ("temp_ads.rt_cust_cleared_stock", "clear_date", Seq("fund_id","stk_cd","clear_date","market"), "clearedstock:rt_cust_cleared_stock"),
    ("temp_ads.rt_cust_cleared_stock_detail", "clear_date", Seq("fund_id","stk_cd","clear_date","market","trd_date","trd_sno"), "clearedstock:rt_cust_cleared_stock_detail")
    // for xianwang
