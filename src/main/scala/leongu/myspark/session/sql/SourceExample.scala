...
object SourceExample extends Logging {
  ...
  def jsonSource(spark: SparkSession): Unit = {
    val peopleDF = spark.read.format("json").load("hdfs://localhost:9000/user/spark/my/people.json")
    peopleDF.show()
  ...
  def csvSource(spark: SparkSession): Unit = {
    val peopleDFCsv = spark.read.format("csv")
      ...
      .option("header", "true")
      .load("hdfs://localhost:9000/user/spark/my/people.csv")
    peopleDFCsv.show()
  ...
  def sqlSource(spark: SparkSession): Unit = {
    val sqlDF = spark.sql("SELECT * FROM parquet.`hdfs://localhost:9000/user/spark/my/users.parquet`")
    sqlDF.show()
  }
  ...
  def parquetSource(spark: SparkSession): Unit = {
    ...
    val peopleDF = spark.read.json("hdfs://localhost:9000/user/spark/my/people.json")
